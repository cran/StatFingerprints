iterative.test <-
function(profil,fact1,level,method)
{
	...
	iterative.wilcox<-function(profil,profn1,profn2)
	{
		for (i in 1:dim(profil)[2]) 
		{
			...
			a[a=="NaN"]<-1
			if (a<0.05) z[i]=0 
			waitGUI(i,c(dim(profil)[2]))
		}
		return(z)
	}
	
	iterative.ttest<-function(profil,profn1,profn2)
	{
		for (i in 1:dim(profil)[2]) 
	...
	iterative.fisher<-function(bin,fact,p)
	{
		...
		for (i in 1:dim(bin)[2])
		{
			...
			waitGUI(i,c(dim(bin)[2]))
		}
		
		return(final)
	}
	
	plot.results<-function(n11,n22,z)
	{
	...
	if (method==2) z=iterative.wilcox(profil,profn1,profn2)
	if (method==1) z=iterative.ttest(profil,profn1,profn2)
	...
	if (method==2) plot.results(n11,n22,z)
	if (method==1) plot.results(n11,n22,z)
