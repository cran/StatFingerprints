...
function()
{
	...
	tt<- tktoplevel()
	tkwm.title(tt, "Compute RDA")
	tkgrid(tklabel(tt, text = "                                                                                      "))
	tkgrid(tklabel(tt, text = "Design your RDA"))
	t1<-tkframe(tt)
	...
	stepwise.rda<-function()
	{
		nb=as.numeric(tclvalue(nb))
		cca1 <- rda(ano2,data=decostand(param.na,"standardize"))
		cca0 <- rda(mat.analyse.na ~ 1, data=decostand(param.na,"standardize"))
		mod <- step(cca0, scope = formula(cca1), test = "perm")
		print(mod)
	}
	
	partial.rda<-function()
	{
		nb=as.numeric(tclvalue(nb))
		z1<-rda(ano2,data=decostand(param.na,"standardize"))
	...
	ttt11<-tkframe(tt)
	b1<- tkbutton(ttt11, text = "Add element to the design of RDA", command = create.model)
	b2<- tkbutton(ttt11, text = "Reset the design of RDA", command = reset.model)
	...
	tkgrid(tkbutton(tt, text = "Stepwise selection of parameters to include in RDA", command =stepwise.rda))
	tkgrid(tkbutton(tt, text = "Compute the RDA", command =compute.rda))
	tkgrid(tkbutton(tt, text = "Plot RDA", command =plot.rda))
	tkgrid(tkbutton(tt, text = "Compute the partialRDA", command =partial.rda))
	...
	tkgrid(tklabel(tt, text = c("Your RDA formula is:")))
	tkfocus(tt)
