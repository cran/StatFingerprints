...
function()
{
	...
	plotord3d <- function()
	{
		...
		if (facte==0 & z=="bestnmds") plot3d(ord[,d1],ord[,d2],ord[,d3],type="s",size=1,col=colo,xlab="1st PC",ylab="2nd PC", zlab="3rd PC",sub=paste("Stress=",c(round(bestnmds$stress,digit=3))),main="Non-metric multidimensional scaling")
		if (facte==0 & z!="bestnmds") plot3d(ord[,d1],ord[,d2],ord[,d3],type="s",size=1,col=colo,xlab="1st PC",ylab="2nd PC", zlab="3rd PC",main="Principal components analysis")
		if (facte!=0 & z=="bestnmds") plot3d(ord[,d1],ord[,d2],ord[,d3],type="s",size=1,col=as.numeric(fact_temp[,facte]),xlab="1st PC",ylab="2nd PC", zlab="3rd PC",sub=paste("Stress=",c(round(bestnmds$stress,digit=3))),main="Non-metric multidimensional scaling")
		if (facte!=0 & z!="bestnmds") plot3d(ord[,d1],ord[,d2],ord[,d3],type="s",size=1,col=as.numeric(fact_temp[,facte]),xlab="1st PC",ylab="2nd PC", zlab="3rd PC",main="Non-metric multidimensional scaling")
		if (facte!=0) plot(1,1,col="white",xlab=NA,ylab=NA,xaxt="n",yaxt="n",axes=FALSE)
		if (facte!=0) legend("topleft",title="Legend of the 3-dimensional plot",legend=c(levels(fact_temp[,facte])),col=c(1:length((levels(fact_temp[,facte])))),pch=rep(16,length(levels(fact_temp[,facte]))))
		if (repe1==1) text3d(ord[,d1],ord[,d2],ord[,d3],rownames(ord))
	}
	
	t7<-tkframe(tt)
	b1 <- tkbutton(t7, text = "Plot", command = plotord3d)
	b3<- tkbutton(t7, text = "New plot window...", command = function() open3d())
	
	save.plotord3d<-function()
	{
	...
	b2<-tkbutton(t7,text="Save picture",command=save.plotord3d)
	b4<-tkbutton(t7,text="Cancel",command=function() tkdestroy(tt))
