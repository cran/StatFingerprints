...
function()
{
	...
	best.ordination.selection<-function()
	{
		...
		if(tex1==2)
		tex<-FALSE
		best.ordination(mat=mat.analyse, return.ordination=repe,text=tex)
	}
	
	t1<-tkframe(tt)
	b1<-tkbutton(t1,text="Compare PCA/nMDS",command=best.ordination.selection)
